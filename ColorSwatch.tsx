...
const PaletteItem: React.FC<PaletteProp> = ({ colors, index, gestureDegree, activeColor, onColorPress }) => {
  ...
  const onAnchorPress = () => gestureDegree.value = gestureDegree.value === 0 ? 90 : 0;
    ...
    <Animated.View style={[styles.paletteContainer, styles.paletteSize, viewStyle]}>
      ...
      <Pressable 
        ...
        <View style={[styles.anchorOuterCircle, { borderColor: activeColor }]}>
          <View style={[styles.anchorInnerCircle, { backgroundColor: activeColor }]} />
        </View>
      </Pressable>
    </Animated.View>
    )
...
const ColorSwatch = () => {
  ...
  const calculateDegree = (event: GestureUpdateEvent<PanGestureHandlerEventPayload>) => {
    'worklet';
    const degree = Math.atan2(PALETTE_HEIGHT - event.y, event.x - PALETTE_WIDTH / 2) * (180 / Math.PI);
